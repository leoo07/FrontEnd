...
const CreateOrderPage = () => {
  ...
  const getAllOrderId = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        ...
        `http://v248s4i.exa-ag.com:8000/sap/opu/odata/sap/ZORD_DETAIL_SRV/order_detailsSet?$format=json`
      ...
      if (data && data?.d?.results) {
        ...
        const mainResult = data?.d?.results;
        ...
        for(let i=0;i<mainResult.length;i++) {
          ...
          const {ORD_ID} = mainResult[i];

 

          if(!allOrders.includes(ORD_ID)) {

            allOrders.push(ORD_ID)
  ...
  const handleSubmit = async (userChosenValue = userText) => {

    try {

      const response = await axios.get(

        `http://v248s4i.exa-ag.com:8000/sap/opu/odata/sap/ZORD_DETAIL_SRV/order_detailsSet?$filter=ORD_ID%20eq%20('${userChosenValue}')&$format=json`
  ...
  return (loading && allOrders.length > 0 ? <>
    ...
    <div className="container">
      ...
      <div className='button-container'>
        ...
        <select name="orders" id="orderId" onChange={e => handleSubmit(e.target.value)}>
        <option value="" disabled selected>Select OrderID</option>
            ...
            <div>
              ...
              <div className="flex-container">
                ...
                <div className="flex-item">
                  ...
                  <h3 className="head1">{numberToWords(allOrders[0]?.TAX_AMT)} Rupees</h3>

                </div>

                <div className="flex-item">                  

                  <h3 className="head">Net Value (Including 10% GST) : {allOrders[0]?.TAX_AMT} </h3>
